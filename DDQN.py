...
class DQN:
    ...
    def update_target_params(self, episode):  # 更新目标网络参数
        if episode % Update_Target_Freq == 0:
            torch.save(self.current_Q_net.state_dict(), 'ddqn_net_params.pkl')  # 保存当前网络参数到本地
            self.target_Q_net.load_state_dict(torch.load('ddqn_net_params.pkl'))  # 上穿当前网络参数并赋给目标网络
